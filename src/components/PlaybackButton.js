...
export default function PlaybackButton({ color, isPlaying, handlePlaybackClick }) {
  const handleClick = () => {
    handlePlaybackClick();
  };
  ...
  return (
    <button
    className={`player__btn player__btn_type_${isPlaying ? 'pause' : 'play'}`}
  {/*
      а вот как тут будет через classnames

      className={classnames('player__btn', {
        ['player__btn_type_pause']: isPlaying,
        ['player__btn_type_play']: !isPlaying
      })}
  ...
  */}
    ...
    {/* тут тоже можно два компонента сделать под отдельные иконки */}
    </button>
  );
}
