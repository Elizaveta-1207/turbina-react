...
export default function MediaPlayer({ songs, currentSong }) {
  ...
  const handleTimelineClick = (evt) => {
    ...
    const songProgressSeconds = Math.round(
      audio.current.duration * songProgressPercentage
    );
  ...
  return (
    <div className={`player player_expanded_${isExpanded}`}>
      ...
      <div className={`player__wrapper_expanded_${isExpanded}`}>
        ...
        {isExpanded && (
          <button className="player__info-button" onClick={toggleContentState}>
            {contentIsText ? "Текст песни" : "Релизы"}
          </button>
        )}
        <button
